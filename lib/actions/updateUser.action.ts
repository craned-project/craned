...
export async function getUserLikePostOrNot(
    ...
    if (post !== null) {
        const check = await User.exists({ id: userid, likes: new mongoose.Types.ObjectId(postid)});
        if (check !== null) {
            console.log(check);
            return true;
        } else return false;
...
export async function handleLike(userId: string, postId: string) {
    try {
        ...
        await User.updateOne(User.findOne({id: userId}), { $addToSet: { likes: postId } });
    } catch (error) {
...
export async function handleUnlike(userId: string, postId: string) {
    try {
        ...
        await User.updateOne(User.findOne({id: userId}), { $pull: { likes: postId } });
    } catch (error) {
